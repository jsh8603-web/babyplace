...
function LoadingSkeleton() {
  return (
    <div className="bg-warm-50 min-h-dvh animate-pulse">
      ...
      {/* Poster skeleton */}
      <div className="w-full h-[250px] bg-warm-200" />
      {/* Content skeleton */}
      <div className="bg-white px-4 py-4 mt-3 space-y-3">
        ...
        <div className="flex gap-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-5 bg-warm-100 rounded w-24" />
          ))}
        </div>
      </div>
      <div className="bg-white px-4 py-4 mt-3 space-y-2">
      ...
      <div className="bg-white px-4 py-4 mt-3">
        <div className="h-12 bg-coral-100 rounded-xl" />\n      </div>
    </div>
  )
}
  ...
  return (
    <div className="bg-warm-50 min-h-dvh flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
        ...
        <p className="text-[17px] font-semibold text-warm-700 mb-2">이벤트를 불러올 수 없습니다</p>
...
export default function EventPage({ params }: EventPageProps) {
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      text: `${data.event.name} - BabyPlace에서 찾은 아이 친화 이벤트`,
