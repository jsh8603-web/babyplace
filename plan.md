...
## 19. To-Do
1. 식당이 안보인다. 크롤링이 현재 등록되어 있는 장소를 한바퀴 안돌았기 때문에 검색 기반의 장소들을 많이 가져오지 못하는 이유인가? 확인하고 해결 필요.
2. 하단 목록의 크기를 줄였다 늘였다 하는게 조작감이 불편하다. 크기 조절이 안되고 배경인 지도가 움직이는 경우도 있다.
3. cc트리거에 배포가 필요한 경우 배포를 할 수 있는 내용을 추가 (커밋 푸시 이후 자동 배포 등).
